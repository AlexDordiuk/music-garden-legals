# Polityka Prywatności

**Aplikacja: Music Garden**
**Data wejścia w życie: 15 lutego 2026 r.**
**Ostatnia aktualizacja: 8 marca 2026 r.**

---

## 1. Wprowadzenie

Witamy w Music Garden ("Aplikacja", "my", "nas", "nasz"). Szanujemy Twoją prywatność i zobowiązujemy się do ochrony Twoich danych osobowych. Niniejsza Polityka Prywatności wyjaśnia, jakie informacje zbieramy, jak je wykorzystujemy oraz jakie prawa przysługują Ci w odniesieniu do Twoich danych.

**Administrator danych:** Oleksandr Dordiuk, Rzeczpospolita Polska
**Kontakt:** alexdordiuk@gmail.com

Korzystając z naszej Aplikacji, wyrażasz zgodę na warunki niniejszej Polityki Prywatności. Jeśli nie zgadzasz się z tymi warunkami, prosimy o niekorzystanie z Aplikacji.

---

## 2. Informacje, które zbieramy

### 2.1. Dane przechowywane lokalnie na Twoim urządzeniu

Music Garden przechowuje następujące informacje **wyłącznie na Twoim urządzeniu**:

**Dane o utworach muzycznych:**
- Nazwy utworów muzycznych
- Nazwy kompozytorów (opcjonalnie)
- Twoje notatki do utworów
- Historia ćwiczeń (data, czas trwania)
- Postęp w nauce

**Ustawienia użytkownika:**
- Wybrany język interfejsu
- Ustawienia powiadomień (włączone/wyłączone, godzina przypomnienia)
- Ustawienia dźwięku i wibracji
- Status ukończenia samouczka

**Dane o postępie:**
- Aktualna seria ćwiczeń (streak)
- Najlepsza seria ćwiczeń
- Data ostatnich ćwiczeń

Te dane **nie są przesyłane na nasze serwery** i są przechowywane wyłącznie lokalnie na Twoim urządzeniu.

### 2.2. Dane o subskrypcjach i zakupach

Do obsługi subskrypcji i zakupów w aplikacji korzystamy z usług podmiotów trzecich:

**RevenueCat:**
- Status Twojej subskrypcji
- Historia zakupów
- Identyfikator klienta (anonimowy)
- Informacje o zakupionych produktach

**Apple App Store:**
- Wszystkie płatności są przetwarzane bezpośrednio przez Apple
- Nie mamy dostępu do Twoich danych płatniczych (numer karty, CVV itp.)

---

## 3. Jak wykorzystujemy Twoje informacje

Wykorzystujemy zebrane informacje do:

- **Zapewnienia funkcjonalności Aplikacji** — zapisywanie Twojego postępu i ustawień
- **Wysyłania powiadomień** — przypomnienia o ćwiczeniach (tylko jeśli udzieliłeś zgody)
- **Zarządzania subskrypcjami** — weryfikacja statusu Twojej subskrypcji w celu zapewnienia dostępu do funkcji premium

---

## 4. Podstawa prawna przetwarzania (RODO Art. 6)

Przetwarzamy Twoje dane na następujących podstawach prawnych:

| Rodzaj danych | Podstawa prawna | Wyjaśnienie |
|---------------|-----------------|-------------|
| Dane lokalne (utwory, ustawienia, postęp) | **Wykonanie umowy** | Niezbędne do zapewnienia podstawowej funkcjonalności Aplikacji |
| Dane o subskrypcji | **Wykonanie umowy** | Niezbędne do realizacji zakupu i zapewnienia funkcji premium |
| Powiadomienia push | **Zgoda** | Wysyłane tylko za Twoją wyraźną zgodą |

---

## 5. Usługi podmiotów trzecich

Aplikacja korzysta z następujących usług podmiotów trzecich:

### 5.1. RevenueCat
- **Cel:** Zarządzanie subskrypcjami i zakupami
- **Polityka prywatności:** [https://www.revenuecat.com/privacy](https://www.revenuecat.com/privacy)

### 5.2. Apple App Store
- **Cel:** Przetwarzanie płatności
- **Polityka prywatności:** [https://www.apple.com/legal/privacy](https://www.apple.com/legal/privacy)

---

## 6. Przechowywanie i bezpieczeństwo danych

### 6.1. Przechowywanie lokalne
Wszystkie Twoje dane osobowe (utwory muzyczne, ustawienia, postęp) są przechowywane **wyłącznie na Twoim urządzeniu** przy użyciu technologii SwiftData firmy Apple. Nie mamy dostępu do tych danych.

### 6.2. Brak synchronizacji w chmurze
Aplikacja **nie synchronizuje** Twoich danych z usługami chmurowymi. Jeśli usuniesz Aplikację, wszystkie lokalne dane zostaną utracone.

### 6.3. Bezpieczeństwo
Twoje dane są chronione standardowymi mechanizmami bezpieczeństwa iOS, w tym szyfrowaniem na poziomie urządzenia.

### 6.4. Okres przechowywania danych
- **Dane lokalne:** Przechowywane na Twoim urządzeniu do momentu usunięcia Aplikacji lub poszczególnych elementów
- **Dane o subskrypcji:** Przechowywane przez RevenueCat przez okres trwania subskrypcji plus okres wymagany prawnie

---

## 7. Twoje prawa (RODO)

Zgodnie z RODO przysługują Ci następujące prawa dotyczące Twoich danych:

### 7.1. Prawo dostępu
Możesz przeglądać wszystkie swoje dane bezpośrednio w Aplikacji.

### 7.2. Prawo do sprostowania
Możesz w dowolnym momencie edytować informacje o utworach muzycznych i ustawieniach.

### 7.3. Prawo do usunięcia ("Prawo do bycia zapomnianym")
- Możesz usuwać poszczególne utwory muzyczne w Aplikacji
- Możesz usunąć wszystkie dane, usuwając Aplikację z urządzenia

### 7.4. Prawo do przenoszenia danych
Ponieważ wszystkie Twoje dane są przechowywane lokalnie na Twoim urządzeniu, możesz uzyskać do nich dostęp poprzez mechanizmy tworzenia kopii zapasowych iOS (kopia zapasowa iCloud lub iTunes/Finder).

### 7.5. Prawo do wniesienia skargi
Jeśli uważasz, że Twoje prawa do ochrony danych zostały naruszone, masz prawo wnieść skargę do organu nadzorczego:

**UODO (Urząd Ochrony Danych Osobowych)**
ul. Stawki 2, 00-193 Warszawa, Polska
Strona internetowa: [https://uodo.gov.pl](https://uodo.gov.pl)
Email: kancelaria@uodo.gov.pl

---

## 8. Powiadomienia push

Aplikacja może wysyłać lokalne powiadomienia, aby przypominać o ćwiczeniach:

- Powiadomienia są wysyłane **tylko za Twoją zgodą**
- Możesz wyłączyć powiadomienia w ustawieniach Aplikacji lub iOS
- Godziny powiadomień można konfigurować w Aplikacji
- Powiadomienia są generowane **lokalnie na Twoim urządzeniu**, a nie z zewnętrznego serwera

---

## 9. Prywatność dzieci

Music Garden jest przeznaczony dla dzieci. Aplikacja nie zbiera żadnych danych analitycznych i nie korzysta z zewnętrznych SDK do śledzenia ani reklam. Wszystkie dane są przechowywane wyłącznie na Twoim urządzeniu.

Aplikacja zawiera weryfikację rodzicielską przed zakupami w aplikacji, aby zapobiec nieautoryzowanym zakupom przez dzieci.

Jeśli jesteś rodzicem lub opiekunem i masz pytania dotyczące danych swojego dziecka, skontaktuj się z nami.

---

## 10. Reklamy

Aplikacja **nie zawiera reklam** i nie korzysta z sieci reklamowych.

---

## 11. Międzynarodowy transfer danych

Dane o subskrypcjach przetwarzane przez RevenueCat mogą być przetwarzane na serwerach znajdujących się poza Twoim krajem, w tym w USA. RevenueCat przestrzega odpowiednich standardów ochrony danych, w tym:
- EU-US Data Privacy Framework (gdzie ma zastosowanie)
- Standardowe klauzule umowne (SCC)

---

## 12. Zmiany w niniejszej Polityce

Możemy od czasu do czasu aktualizować niniejszą Politykę Prywatności. O istotnych zmianach powiadomimy poprzez:
- Aktualizację daty "Ostatnia aktualizacja" na początku tego dokumentu
- Powiadomienia w Aplikacji (jeśli to konieczne)

Zalecamy okresowe przeglądanie niniejszej Polityki, aby być na bieżąco z aktualnymi informacjami.

---

## 13. Prawo właściwe

Niniejsza Polityka Prywatności podlega prawu Rzeczypospolitej Polskiej oraz obowiązującemu prawodawstwu Unii Europejskiej, w tym Ogólnemu Rozporządzeniu o Ochronie Danych (RODO).

---

## 14. Dane kontaktowe

Jeśli masz pytania dotyczące niniejszej Polityki Prywatności lub przetwarzania Twoich danych, skontaktuj się z nami:

**Administrator danych:** Oleksandr Dordiuk
**Email:** alexdordiuk@gmail.com
**Kraj rejestracji:** Rzeczpospolita Polska

---

## 15. Zgoda

Korzystając z Aplikacji Music Garden, potwierdzasz, że:
- Przeczytałeś niniejszą Politykę Prywatności
- Rozumiesz, w jaki sposób Twoje dane są zbierane i wykorzystywane
- Zgadzasz się z warunkami niniejszej Polityki

---

*© 2026 Music Garden. Wszelkie prawa zastrzeżone.*

---

[← Powrót do strony głównej](.)
